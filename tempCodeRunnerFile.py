...
def add_contact():
    ...
    try:
        cursor.execute("""INSERT INTO contacts (FirstName, LastName, DOB, PhoneNumber, EmailID, Gender) VALUES (%s, %s, %s, %s, %s, %s)""",
                       (first_name, last_name, dob, phone_number, email_id, gender))
        db.commit()
...
def extract_contact():
    ...
    try:
        ...
        if contact:
            ...
            tmsg.showwarning("No Record Found", "No Record Found With The Provided Number !!")
            clear_entries()  # Clear the entry fields if no record is found
    except Exception as e:
...
def update_contact():
    ...
    try:
        cursor.execute("""UPDATE contacts SET FirstName=%s, LastName=%s, DOB=%s, EmailID=%s, Gender=%s WHERE PhoneNumber=%s""",
                       (first_name, last_name, dob, email_id, gender, phone_number))
        db.commit()
        if cursor.rowcount == 0:
            tmsg.showwarning("No Record Found", "No Record Found With The Provided Number !!")
        else:
        ...
        tmsg.showerror("Database Error", f"Error Updating Contact In Database: {e}")
...
try:
    db = mysql.connector.connect(host="localhost", user="root", password="1107", database="contact_management_system")
    cursor = db.cursor()
